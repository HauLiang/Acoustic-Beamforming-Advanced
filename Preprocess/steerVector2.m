function [g, w] = steerVector2(plane_distance, frequencies, scan_limits, scan_resolution, mic_positions, c, mic_centre)
%
% This code implements the generation of the steering vector -- Formulation II
%
...
for K = 1:N_freqs
    ...
    for m = 1:N_mic
        ...
        % Calculate the weighted steering vecor (formulation II)
        w(:,:,m, K) = (r_scan_to_mic(:,:,m)./r_scan_to_mic_centre).*exp(-1j*omega.*(r_scan_to_mic(:,:,m)-r_scan_to_mic_centre)./c);
        
        % Calculate the steering vector (formulation II)
        g(:,:,m, K) = 1./((r_scan_to_mic(:,:,m)./r_scan_to_mic_centre)).*exp(-1j*omega.*(r_scan_to_mic(:,:,m)-r_scan_to_mic_centre)./c);
