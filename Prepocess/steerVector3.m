function [g, w] = steerVector3(plane_distance, frequencies, scan_limits, scan_resolution, mic_positions, c, mic_centre)
%
% This code implements the generation of the steering vector -- Formulation III
%
...
sum_r_scan_to_mic = zeros(N_X, N_Y);

% Calculate the reciprocal of the sum of squares of the distance from the scan plane to microphone array
for m = 1:N_mic
    ...
    % Calculate the reciprocal of the sum of squares
    sum_r_scan_to_mic = sum_r_scan_to_mic + 1./(r_scan_to_mic(:,:,m).^2);
end
...
for K = 1:N_freqs
    ...
    for m = 1:N_mic
        ...
        % Calculate the weighted steering vecor (formulation III)
        w(:,:,m, K) = N_mic./(r_scan_to_mic(:,:,m).*r_scan_to_mic_centre)./sum_r_scan_to_mic.*exp(-1j*omega.*(r_scan_to_mic(:,:,m)-r_scan_to_mic_centre)./c);
        
        % Calculate the steering vector (formulation III)
        g(:,:,m, K) = 1./(N_mic./(r_scan_to_mic(:,:,m).*r_scan_to_mic_centre)./sum_r_scan_to_mic).*exp(-1j*omega.*(r_scan_to_mic(:,:,m)-r_scan_to_mic_centre)./c);
