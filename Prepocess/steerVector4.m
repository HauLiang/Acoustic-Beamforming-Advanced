function [g, w] = steerVector4(plane_distance, frequencies, scan_limits, scan_resolution, mic_positions, c, mic_centre)
%
% This code implements the generation of the steering vector -- Formulation IV
%
...
for K = 1:N_freqs
    ...
    for m = 1:N_mic
        ...
        % Calculate the weighted steering vecor (formulation IV)
        w(:,:,m, K) = N_mic./r_scan_to_mic(:,:,m)./(sqrt(N_mic.*sum_r_scan_to_mic)).*exp(-1j*omega.*(r_scan_to_mic(:,:,m)-r_scan_to_mic_centre)./c);
        
        % Calculate the steering vector (formulation IV)
        g(:,:,m, K) = 1./(N_mic./r_scan_to_mic(:,:,m)./(sqrt(N_mic.*sum_r_scan_to_mic))).*exp(-1j*omega.*(r_scan_to_mic(:,:,m)-r_scan_to_mic_centre)./c);
